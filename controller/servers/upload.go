...
// 接收两个参数 一个文件流 一个 bucket 你的七牛云标准空间的名字
func UploadLocal(filePath string, bucket string, urlPath string, fileKey string) (err error, path string, key string) {
	putPolicy := storage.PutPolicy{
		Scope: fmt.Sprintf("%s:%s", bucket, fileKey),
	}
	mac := qbox.NewMac(accessKey, secretKey)
	upToken := putPolicy.UploadToken(mac)
	cfg := storage.Config{}
	...
	err = formUploader.PutFile(context.Background(), &ret, upToken, fileKey, filePath, &putExtra)
	if err != nil {
		fmt.Println(err)
